...
impl Env {
    ...
    fn run_diff(&self, args: &[&str]) -> std::process::Output {
        Command::new(BIN)
            .current_dir(&self.source)
            .arg("diff")
            .args(args)
    ...
    fn run_pr(&self, args: &[&str]) -> std::process::Output {
        Command::new(BIN)
            .current_dir(&self.source)
            .arg("pr")
            .args(args)
...
fn two_transform_config() -> &'static str {
    "xform1:\n  target: ../target.git\n  rule:\n    command: \"true\"\nxform2:\n  target: ../target2.git\n  rule:\n    command: \"true\"\n"
}
...
fn set_cache_remote_url(env: &Env, transform: &str, url: &str) {
    let cache = env
        .source
        .join(".git/git-xf")
